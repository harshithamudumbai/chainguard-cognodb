...
const logger = (0, pino_1.default)({ name: "seed" });
const runSeed = async () => {
    ...
    try {
        ...
        // Create constraints
        await session.executeWrite((tx) => tx.run(`CREATE CONSTRAINT product_id_unique IF NOT EXISTS FOR (p:Product) REQUIRE p.id IS UNIQUE;`));
        await session.executeWrite((tx) => tx.run(`CREATE CONSTRAINT component_id_unique IF NOT EXISTS FOR (c:Component) REQUIRE c.id IS UNIQUE;`));
        await session.executeWrite((tx) => tx.run(`CREATE CONSTRAINT supplier_id_unique IF NOT EXISTS FOR (s:Supplier) REQUIRE s.id IS UNIQUE;`));
        await session.executeWrite((tx) => tx.run(`CREATE CONSTRAINT facility_id_unique IF NOT EXISTS FOR (f:Facility) REQUIRE f.id IS UNIQUE;`));
        await session.executeWrite((tx) => tx.run(`CREATE CONSTRAINT country_code_unique IF NOT EXISTS FOR (c:Country) REQUIRE c.code IS UNIQUE;`));
        await session.executeWrite((tx) => tx.run(`CREATE CONSTRAINT risk_event_id_unique IF NOT EXISTS FOR (r:RiskEvent) REQUIRE r.id IS UNIQUE;`));
        logger.info("Constraints created.");
        // Clear existing application data (only nodes with our labels to avoid destroying everything)
        await session.executeWrite((tx) => tx.run(`MATCH (n) WHERE n:Product OR n:Component OR n:Supplier OR n:Facility OR n:Country OR n:RiskEvent DETACH DELETE n;`));
    ...
    `, { data: supplierHeadquarteredIn }));
        ...
        await session.executeWrite((tx) => tx.run(`
      UNWIND $data AS row 
      MATCH (r:RiskEvent {id: row.r})
      CALL apoc.cypher.doIt(
        "MATCH (target:" + row.targetLabel + " {id: $targetId}) " +
        "CREATE (r)-[:AFFECTS {impactLevel: $impactLevel, estimatedDelayDays: $estimatedDelayDays}]->(target)",
        {r: r, targetId: row.targetId, impactLevel: row.impactLevel, estimatedDelayDays: row.estimatedDelayDays}
      ) YIELD value RETURN value
    `, { data: riskAffects }).catch(() => {
            // Fallback if apoc is not installed, do it manually
            tx.run(`
        UNWIND $data AS row
